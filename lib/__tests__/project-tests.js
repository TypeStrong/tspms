...
var WorkingSetMock = require('./workingSetMock');
var project = require('../project');
var utils = require('../utils');
describe('project test', function () {
    var fileSystemMock, workingSetMock, typeScriptProject;
    var defaultLibLocation = '/lib.d.ts';
    function createProject(baseDir, config, init) {
        if (init === void 0) { init = true; }
        typeScriptProject = project.createProject(baseDir, utils.assign({}, utils.typeScriptProjectConfigDefault, config), fileSystemMock, workingSetMock, defaultLibLocation);
        if (init) {
            typeScriptProject.init();
        }
    }
    ;
    function expectToBeEqualArray(actual, expected) {
        expect(actual.sort()).toEqual(expected.sort());
    }
    function testWorkingSetOpenCorrespondance() {
        var languageServiceHost = typeScriptProject.getLanguageServiceHost();
        Object.keys(typeScriptProject.getProjectFilesSet()).forEach(function (fileName) {
            expect(languageServiceHost.getScriptIsOpen(fileName)).toBe(workingSetMock.files.indexOf(fileName) !== -1);
        });
    }
    function getProjectFileContent(fileName) {
    ...
    beforeEach(function () {
        fileSystemMock = new FileSystemMock(), workingSetMock = new WorkingSetMock();
    });
    afterEach(function () {
    ...
    describe('filesystem change handling', function () {
        ...
        it('should remove referenced files from the project when a source file referencing it is deleted, ' + 'only if it is not referenced by another file', function () {
            fileSystemMock.setFiles({
                '/src/file1.ts': 'import test = require("../other/file3")',
        ...
        xit('should create a new typescript factory instance if a typescript path is specified', function () {
            var mockTypeScript = jest.genMockFromModule('typescript');
    ...
    describe('project update update', function () {
        ...
        it('should update compilerOptions if compiler options does have changed', function () {
            expect(typeScriptProject.getLanguageServiceHost().getCompilationSettings().target).toBe(1 /* ES5 */);
            updateProject({
                target: 'es3',
                module: 'commonjs',
            ...
            jest.runAllTimers();
            expect(typeScriptProject.getLanguageServiceHost().getCompilationSettings().target).toBe(0 /* ES3 */);
        });
    ...
    describe('getProjectFileKind', function () {
        it('should return \'SOURCE\' if the file path match the \'sources\' section of the given config', function () {
            ...
            jest.runAllTimers();
            expect(typeScriptProject.getProjectFileKind('/src/file1.ts')).toBe(1 /* SOURCE */);
        });
        it('should return \'REFERENCE\' if the file is a referenced file', function () {
            ...
            jest.runAllTimers();
            expect(typeScriptProject.getProjectFileKind('/other/file2.ts')).toBe(2 /* REFERENCE */);
        });
        it('should return \'NONE\' if the file is a nor a part of the project', function () {
            ...
            jest.runAllTimers();
            expect(typeScriptProject.getProjectFileKind('/other/file2.ts')).toBe(0 /* NONE */);
        });
    });
