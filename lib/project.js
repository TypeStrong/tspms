...
function createProject(baseDirectory, config, fileSystem, workingSet) {
    //-------------------------------
    //  variables
    //-------------------------------
    /**
    ...
    function documentEditedHandler(record) {
        queue.then(function () {
            if (projectFilesSet[record.path]) {
                var oldPaths = utils.createMap(getReferencedOrImportedFiles(record.path)), lastChange;
    ...
    function init() {
        ...
        return queue.reset(new promise.Promise(function (resolve, reject) {
            if (!typescriptPath) {
                ...
                resolve(compilerManager.acquireCompiler(_config.typescriptPath).catch(function (e) {
            ...
            languageService = typeScriptInfo.typeScript.createLanguageService(languageServiceHost, info.documentRegistry);
